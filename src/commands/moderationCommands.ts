...
type ModerationCommandDeps = {
    requireAdministrator: (interaction: ChatInputCommandInteraction) => string | null;
    ensureGuildModeration: (guildId: string) => GuildModerationState;
    saveModerationStore: () => void;
    rejectIfDuplicateCommand: (interaction: ChatInputCommandInteraction, fingerprint: string) => string | null;
    sendModLog: (guildId: string, title: string, fields: Array<{ name: string; value: string; inline?: boolean }>) => Promise<void>;
    parseDurationMs: (raw: string) => number | null;
    getAvatar: (user: User) => string;
};

export function buildModerationCommandHandlers(deps: ModerationCommandDeps): Record<string, CommandHandler> {
    return {
        ...
        lockdownnotice: async interaction => {
            ...
            const embed = new EmbedBuilder()
                ...
                .addFields(
                    { name: "🚫 WHAT HAPPENS IF YOU TYPE HERE", value: "Your message is deleted and your account is banned immediately.", inline: false },
                    { name: "🛡️ WHY THIS EXISTS", value: "To stop bot spam, invite spam, and mass-raiding activity before it spreads.", inline: false },
                    { name: "👮 WHO MAY SPEAK", value: "Administrators only.", inline: false }
                )
        ...
        warn: async interaction => {
            ...
            if (!state.warnings[target.id]) state.warnings[target.id] = [];
        ...
        warnings: async interaction => {
            ...
            if (!list.length) return `✅ ${target.username} has no warnings.`;
        ...
        clearwarnings: async interaction => {
            ...
            if (clearWarnDedupeError) return clearWarnDedupeError;
            const state = deps.ensureGuildModeration(interaction.guildId!);
        ...
        tempban: async interaction => {
            ...
            if (tempbanDedupeError) return tempbanDedupeError;
        ...
        purge: async interaction => {
            ...
            if (purgeDedupeError) return purgeDedupeError;
            if (amount < 2 || amount > 100) return "Amount must be between 2 and 100.";
            if (!interaction.channel || !interaction.channel.isTextBased()) return "This command requires a text channel.";
            if (!("bulkDelete" in interaction.channel)) return "This channel does not support bulk delete.";
            ...
            if (!candidates.length) return "No matching messages found.";
        ...
        announce: async interaction => {
            ...
            const dedupeError = deps.rejectIfDuplicateCommand(
                interaction,
                `announce:${selectedChannel?.id || interaction.channelId}:title:${title}:desc:${description}:color:${colorRaw}:footer:${footer}:image:${imageUrl}`
            );

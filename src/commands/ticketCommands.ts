...
import { summarizeTicketAnalytics } from "../services/ticketAnalytics";
import type { TicketTranscriptStub } from "../services/ticketTranscript";

type CommandHandler = (interaction: ChatInputCommandInteraction) => Promise<string>;
...
type TicketRecord = {
    ...
    workflowStatus: "new" | "responded" | "waiting_user" | "escalated" | "resolved";
    ...
    transcript: TicketTranscriptStub | null;
    createdAt: number;
    updatedAt: number;
...
type TicketPanelUpsertResult = {
    ok: boolean;
    action?: "updated" | "posted";
    error?: string;
};

type TicketCommandDeps = {
    requireGuild: (interaction: ChatInputCommandInteraction) => string | null;
    requireAdministrator: (interaction: ChatInputCommandInteraction) => string | null;
    canManageTicketActions: (member: GuildMember) => boolean;
    resolveTicketTargetChannelId: (interaction: ChatInputCommandInteraction) => { channelId: string | null; error: string | null };
    ensureTrackedTicketByChannelId: (guild: Guild, channelId: string, actorId: string) => Promise<TicketRecord | null>;
    createTicketChannel: (guild: Guild, ownerId: string, reason: string, priority: "low" | "normal" | "high") => Promise<{ channelId?: string; error?: string }>;
    findTicketByChannel: (channelId: string) => TicketRecord | null;
    buildTicketCommandEmbedPayload: (title: string, description: string, ticket?: TicketRecord, extraFields?: EmbedField[]) => string;
    upsertTicketPanelInChannel: (guild: Guild, channelId: string) => Promise<TicketPanelUpsertResult>;
    claimTicketChannel: (guild: Guild, channelId: string, claimedById: string) => Promise<string>;
    handleTicketAssignCommand: (interaction: ChatInputCommandInteraction, usedAlias?: boolean) => Promise<string>;
    normalizeTicketPriority: (priority: unknown) => "low" | "normal" | "high";
    normalizeTicketStatus: (status: unknown) => "open" | "claimed" | "archived" | "resolved";
    normalizeTicketWorkflowStatus: (status: unknown) => "new" | "responded" | "waiting_user" | "escalated" | "resolved";
    rejectIfDuplicateCommand: (interaction: ChatInputCommandInteraction, fingerprint: string) => string | null;
    setTicketWorkflowStatus: (channelId: string, workflowStatus: "new" | "responded" | "waiting_user" | "escalated" | "resolved") => TicketRecord | null;
    updateTicketOpsPanelMessage: (guild: Guild, ticket: TicketRecord) => Promise<void>;
    sendTicketLog: (guildId: string, title: string, fields: EmbedField[]) => Promise<void>;
    appendAuditEvent: (eventType: string, payload: Record<string, unknown>) => void;
    getTicketNextActionHint: (ticket: TicketRecord) => string;
    canTransitionTicketStatus: (fromStatus: unknown, toStatus: unknown) => boolean;
    reopenTicketByChannel: (channelId: string, reopenedById: string, reopenReason: string) => TicketRecord | null;
    findOpenTicketByChannel: (channelId: string) => TicketRecord | null;
    closeTicketChannel: (guild: Guild, channelId: string, closedById: string, closeReason: string) => Promise<string>;
    findArchivedTicketByChannel: (channelId: string) => TicketRecord | null;
    resolveTicketChannel: (guild: Guild, channelId: string, resolverId: string, resolvedReason: string) => Promise<string>;
    ensureTicketConfig: (guildId: string) => TicketConfig;
    saveTicketStore: () => boolean;
    pruneDeletedTicketRecords: (guild: Guild) => Promise<number>;
    pruneInaccessibleOwnerTicketRecords: (guild: Guild) => Promise<number>;
    getTicketSlaState: (ticket: TicketRecord) => TicketSlaState;
    ticketStore: { tickets: TicketRecord[] };
    TICKET_HANDLER_ROLE_ID: string;
    TICKET_DEFAULT_CATEGORY_ID: string;
};

export function buildTicketCommandHandlers(deps: TicketCommandDeps): Record<string, CommandHandler> {
    return {
        ticket: async interaction => {
            ...
            return deps.buildTicketCommandEmbedPayload(
                "🎫 Ticket Opened",
                "Your support ticket is live and queued for FN Support handling.",
                ticket || undefined,
                [
        ...
        ticketpanel: async interaction => {
            ...
            if (!result.ok) return result.error || "Unable to upsert ticket panel.";
        ...
        claimticket: async interaction => {
            ...
            if (target.error || !target.channelId) return target.error || "Unable to resolve the target ticket channel.";

            const tracked = await deps.ensureTrackedTicketByChannelId(interaction.guild!, target.channelId, interaction.user.id);
            if (!tracked) return "This channel is not a tracked ticket and could not be imported. Check the channel ID or run /tickets.";
            const msg = await deps.claimTicketChannel(interaction.guild!, target.channelId, interaction.user.id);
            if (!msg.toLowerCase().includes("claimed")) return msg;
            const claimed = deps.findTicketByChannel(target.channelId);
            return deps.buildTicketCommandEmbedPayload(
                "🛠️ Ticket Claimed",
                "Claim succeeded. Live ticket panel status was updated.",
                claimed || tracked,
                [
        ...
        ticketstatus: async interaction => {
            ...
            if (!ticket) return "This channel is not a tracked ticket and could not be imported. Use ticket_channel_id or run /tickets.";
            ...
            if (statusDedupeError) return statusDedupeError;
            ...
            if (!updated) return "Unable to update status for this ticket.";
            ...
            return deps.buildTicketCommandEmbedPayload(
                ...
                "Workflow status changed successfully.",
                updated,
                [
                    { name: "👤 Updated By", value: `<@${interaction.user.id}>`, inline: true },
        ...
        reopenticket: async interaction => {
            ...
            const tracked = await deps.ensureTrackedTicketByChannelId(interaction.guild!, target.channelId, interaction.user.id);
            if (!tracked) return "This channel is not a tracked ticket and could not be imported. Use ticket_channel_id or run /tickets.";

            const ticket = deps.findTicketByChannel(target.channelId);
            if (!ticket) return "Unable to locate ticket record for this channel.";
            ...
            if (dedupeError) return dedupeError;

            const reopened = deps.reopenTicketByChannel(target.channelId, interaction.user.id, reason);
            if (!reopened) return "Unable to reopen this ticket right now.";
            ...
            return deps.buildTicketCommandEmbedPayload(
                "🔁 Ticket Reopened",
                "Archived ticket returned to active workflow and support queue.",
                reopened,
                [
                    { name: "👤 Reopened By", value: `<@${interaction.user.id}>`, inline: true },
                    { name: "📝 Reason", value: reason, inline: false },
        ...
        closeticket: async interaction => {
            ...
            if (!ticket) return "This is not an open ticket channel.";
            ...
            if (closeDedupeError) return closeDedupeError;
            ...
            if (!archived) return msg;
            return deps.buildTicketCommandEmbedPayload(
                "🔒 Ticket Archived",
                "Ticket moved to archive hold queue and set read-only for the owner.",
                archived,
                [
        ...
        resolveticket: async interaction => {
            ...
            if (!ticket) return "This is not an archived ticket channel.";
            ...
            if (resolveDedupeError) return resolveDedupeError;
            ...
            return deps.buildTicketCommandEmbedPayload(
                ...
                "Ticket was resolved and permanently closed.",
                resolved,
                [
        ...
        ticketconfig: async interaction => {
            ...
            if (ticketConfigDedupeError) return ticketConfigDedupeError;
        ...
        tickets: async interaction => {
            ...
            const visible = deps.ticketStore.tickets
                ...
                .slice(0, 20);
            if (!visible.length) return "No active or archived tickets right now.";
        ...
        ticketanalytics: async interaction => {
            ...
            const summary = summarizeTicketAnalytics(deps.ticketStore.tickets, guild.id, Date.now());
            const fmt = (value: number | null, unit: string) => value === null ? "n/a" : `${value}${unit}`;

...
exports.classifyTicketCategory = classifyTicketCategory;
exports.getKbSuggestions = getKbSuggestions;
exports.findPotentialDuplicateTickets = findPotentialDuplicateTickets;
exports.getTicketSlaPolicy = getTicketSlaPolicy;
exports.pickLeastLoadedAssignee = pickLeastLoadedAssignee;
...
function classifyTicketCategory(reason) {
    ...
    if (/appeal|ban|mute|timeout/.test(text))
        return "appeal";
    if (/billing|payment|purchase|refund|charge/.test(text))
        return "billing";
    if (/account|login|password|verify|2fa/.test(text))
        return "account";
    if (/report|abuse|cheat|scam|harass/.test(text))
        return "report";
    if (/bug|error|broken|issue|glitch|crash/.test(text))
        return "bug";
    return "general";
}
function getKbSuggestions(reason) {
...
function findPotentialDuplicateTickets(input) {
...
function getTicketSlaPolicy(category, priority) {
...
function pickLeastLoadedAssignee(handlerIds, tickets) {
    ...
    for (const ticket of tickets) {
        ...
        if (ticket.status === "resolved")
            continue;
        if (!load.has(ticket.assignedToId))
            continue;
...
function canReopenTicket(reopenUntilAt, now = Date.now()) {
    if (!reopenUntilAt)
        return false;
    return now <= reopenUntilAt;
}
function extractSearchableTicketText(ticket) {
    const notesText = (ticket.notes || []).map(note => note.note).join(" ");
    return `${ticket.reason || ""} ${ticket.category || ""} ${notesText}`.toLowerCase();
}
function shouldPurgeResolvedTicket(ticket, retentionDays, now = Date.now()) {
    if (ticket.status !== "resolved")
        return false;
    if (!ticket.resolvedAt || retentionDays <= 0)
        return false;

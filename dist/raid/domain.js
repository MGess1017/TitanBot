...
exports.calculateMapReputationGain = calculateMapReputationGain;
exports.getBossCombatProfile = getBossCombatProfile;
exports.getBossCombatModifiers = getBossCombatModifiers;
...
function getMapReputationTier(points) {
    const safePoints = Math.max(0, Math.floor(points || 0));
    return [...exports.MAP_REPUTATION_TIERS].reverse().find(tier => safePoints >= tier.threshold) || exports.MAP_REPUTATION_TIERS[0];
}
function getMapReputationProgress(points) {
    const safePoints = Math.max(0, Math.floor(points || 0));
    const tier = getMapReputationTier(safePoints);
    const nextTier = exports.MAP_REPUTATION_TIERS.find(entry => entry.level === tier.level + 1) || null;
    if (!nextTier)
        return { points: safePoints, tier, nextTier: null, progressPct: 100, pointsToNext: 0 };
...
function calculateMapReputationGain(input) {
...
function getBossCombatProfile(name) {
    return exports.BOSS_COMBAT_PROFILES[name] || { traits: ["relentless"], phases: DEFAULT_BOSS_PHASES };
}
function getBossCombatModifiers(name, approachKey) {
    const profile = getBossCombatProfile(name);
    const traits = profile.traits.map(key => exports.BOSS_TRAITS[key]);

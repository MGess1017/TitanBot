...
function getRaidLoadoutBonus(input) {
    ...
    if (selectedWeaponId) {
        ...
        weapon = getBestOwnedGear(userId, catalog_1.WEAPON_IDS, "raidAttack");
    }
    if (selectedArmorId) {
        const def = catalog_1.ITEM_DEFS[selectedArmorId];
        ...
        armor = getBestOwnedGear(userId, catalog_1.ARMOR_IDS, "raidDefense");
    }
    ...
    const notes = [];
    if (weapon) {
        const trait = domain_1.WEAPON_TRAITS[weapon.id];
        if (trait?.successBonus)
            attackBoost += trait.successBonus;
        if (trait?.tokenBonus)
            tokenBoost += trait.tokenBonus;
        if (trait?.xpBonus)
            xpMultiplier += trait.xpBonus;
        const c = trait?.conditionSuccess?.[condition.key];
        if (c)
            attackBoost += c;
        if (trait?.note)
            notes.push(`${weapon.name}: ${trait.note}`);
    }
    if (armor) {
        const trait = domain_1.ARMOR_TRAITS[armor.id];
        if (trait?.successBonus)
            attackBoost += trait.successBonus;
        if (trait?.tokenBonus)
            tokenBoost += trait.tokenBonus;
        if (trait?.xpBonus)
            xpMultiplier += trait.xpBonus;
        if ((0, domain_1.hasConditionImmunity)(trait, condition.key)) {
        ...
        if (trait?.note)
            notes.push(`${armor.name}: ${trait.note}`);
    }
    if (weapon && armor) {
    ...
    if (selectedWeaponId && weapon)
        notes.push(`Manual weapon selected: ${weapon.name}`);
    if (selectedArmorId && armor)
        notes.push(`Manual armor selected: ${armor.name}`);
    return {
...
function formatLoadoutSummary(input) {
...
function rollRaidLoot(input) {
    ...
    const tuning = domain_1.RAID_LOOT_TUNING_BY_DIFFICULTY[mapCfg.difficulty];
    ...
    const rollCount = success
        ? tuning.successBaseRolls + (tension === "high" ? tuning.successHighTensionBonusRolls : 0) + ((0, domain_1.isAdvancedRaidDifficulty)(mapCfg.difficulty) ? tuning.successHardMapBonusRolls : 0)
    ...
    ].filter(entry => catalog_1.COLLECTIBLE_ITEM_IDS.includes(entry.id));
    const ultraCollectiblePool = [
    ...
    ].filter(entry => catalog_1.ULTRA_RARE_COLLECTIBLE_IDS.includes(entry.id));
    ...
    if (success && Math.random() < rareDiscoveryChance) {
        const discoveryTable = Math.random() < 0.5 ? domain_1.RAID_WEAPON_DISCOVERY_TABLE : domain_1.RAID_ARMOR_DISCOVERY_TABLE;
    ...
    if (success && Math.random() < apexDiscoveryChance) {
        const apexTable = Math.random() < 0.5 ? domain_1.RAID_APEX_WEAPON_DISCOVERY_TABLE : domain_1.RAID_APEX_ARMOR_DISCOVERY_TABLE;
